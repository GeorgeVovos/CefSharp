        ...
        try
        {
            ...
            if($file.StartsWith("CefSharp.Common") -and $ARCHES.Count -lt $SupportedArches.Count)
            {
                ...
                foreach($a in $removeArches)
                {
                    ...
                    {
                        # Remove BTL.cef.redist dependency
                        $depNode =  $NupkgXml.package.metadata.dependencies.group.dependency | Where-Object {$_.Attributes["id"].Value.Equals("BTL.cef.redist." + $a) };
...
if($IsNetCoreBuild)
{
    ...
    $NupkgFiles = @('BTL.CefSharp.Common.NETCore.nuspec', 'BTL.CefSharp.WinForms.NETCore.nuspec', 'BTL.CefSharp.Wpf.NETCore.nuspec','BTL.CefSharp.OffScreen.NETCore.nuspec')
